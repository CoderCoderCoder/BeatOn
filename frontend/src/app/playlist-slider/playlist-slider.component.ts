...
export class PlaylistSliderComponent implements OnInit {
    ...
    getBackground(item) {
        ...
        return (
            ...
            '/host/beatsaber/playlist/cover?playlistid=' +
            fixedUri +
            '&update=' +
            this.updateCounterHack +
            ')'
        );
    }
    ...
    playlistDrop(item: BeatSaberPlaylist, evt) {
        ...
        var oldIndex = oldPlaylist.SongList.findIndex(
            x =>
                evt.item.element.nativeElement.attributes['songid'] &&
                x.SongID == evt.item.element.nativeElement.attributes['songid'].value
        );
